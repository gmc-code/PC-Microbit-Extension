...
class MOVEMotorMotors:
    ...
    def stop_motor(self, motor, motor_rev):
        stop_buffer = bytearray([motor, 0])
        i2c.write(CHIP_ADDR, stop_buffer, False)
        stop_buffer[0] = motor_rev
        i2c.write(CHIP_ADDR, stop_buffer, False)

    def stop_left(self):
        self.stop_motor(LEFT_MOTOR, LEFT_MOTOR_REV)

    def stop_right(self):
        self.stop_motor(RIGHT_MOTOR, RIGHT_MOTOR_REV)
